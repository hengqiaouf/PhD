...
class FirmaData_onesubject(Dataset):
    def __init__(self, data_folder_dir, sub_id, train_part, val_part, test_part, window_size, subset='train',
                 pre_process=True):
        matfiles = []
...
class FirmaData_select_subjects(Dataset):
    def __init__(self, data_folder_dir, window_size, train_part, val_part, test_part, subject_list, subset='train',
                 pre_process=True):  # subset = train, test, val
        matfiles = []
        assert train_part + val_part + test_part == 1  # partition of three sub datasets
        self.window_size = window_size
        for f in os.listdir(data_folder_dir):
...
class FirmaData_all_subjects(Dataset):
    def __init__(self, data_folder_dir, window_size, train_part, val_part, test_part, subset='train',
                 pre_process=True):  # subset = train, test, val
        matfiles = []
        assert train_part + val_part + test_part == 1  # partition of three sub datasets
        self.window_size = window_size
